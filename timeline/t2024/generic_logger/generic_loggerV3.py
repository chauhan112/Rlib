...
class ModelFuncsDictionary:
    ...
    def metaInfo_update(self, tableName, key, new_value):
        uuid = self.logger_get_uuid(tableName)
...
def Pagination():
    def _pagination(state):
        ...
        container = Utils.container([prev, btnsContainer,nextBtn, label, pageInput, goBtn])
        def selected(wid):
...
def ResultDisplayers():
    def _resultDisplayers(state):
        ...
        def set_results(results, newPageNr = 1, reverseIt = True):
            ...
            if state.process.totalPagesNr < newPageNr:
                newPageNr = 1
            state.process.pageNr = newPageNr
            state.process.paginations.handlers.selectWithVal(str(newPageNr))
            state.handlers.render()
        def btnMaker(ele):
        ...
        def button_state_update(btn, info):
            ...
            btn.outputs.layout.tooltip = state.handlers.name_getter(ele)
        ...
        container = Utils.container([paginations.views.container, btns], className="flex flex-column")
        addToNameSpace(state, locals(), ["state", "addToNameSpace"])
    state = NameSpace()
...
def TabComponent():
    def tabElement(state):
        ...
        tabElement = Utils.container([tabTitle, closeBtn], className = "bg-blue-light w-fit br-5px p0 ml-2px")
...
def KeyValueComponent():
    def _temp(state):
        ...
        import json
        def get_values():
            return moreInfoLay.state.controller._basic._model.content
